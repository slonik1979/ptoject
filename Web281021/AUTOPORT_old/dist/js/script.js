$(document).ready(function () {
  ...
  $('.carousel__inner').slick({
    ...
    autoplaySpeed: 1000,
    prevArrow:
      '<button type="button" class="slick-prev"><img src="../icons/left.svg" alt="" /></button>',
    nextArrow:
      '<button type="button" class="slick-next"><img src="../icons/right.svg" alt="" /></button>',
  ...
  function toggleSlide(item) {
    $(item).each(function (i) {
      $(this).on('click', function (e) {
        ...
        $('.catalog-item__block')
          .eq(i)
          .toggleClass('catalog-item__block_active');
  ...
  function getZero(num) {
    // добавляем 0, если дни часы минуты секунды меньше 10
    if (num >= 0 && num < 10) {
      return `0${num}`;
    } else {
      return num;
